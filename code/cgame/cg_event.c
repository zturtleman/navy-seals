...
void CG_EntityEvent( centity_t *cent, vec3_t position ) {
	...
	case EV_FALL_SHORT:
		...
		if ( ( cg.DeafTime < cg.time ) ) {
			trap_S_StartSoundExtended( NULL, 1.0, SEALS_FOOTSTEP_FALLOFF, 1.0f, es->number, CHAN_AUTO, cgs.media.landSound );
		}
	...
	case EV_FALL_DEATH:
		...
		if ( ( cg.DeafTime < cg.time ) ) {
			trap_S_StartSoundExtended( NULL, 1.0, SEALS_FOOTSTEP_FALLOFF, 1.0f, es->number, CHAN_AUTO, trap_S_RegisterSound( "sound/actors/player/fall1.wav", qfalse ) );
		}
	...
	case EV_WATER_CLEAR:
		...
		if ( ( cg.DeafTime < cg.time ) ) {
			trap_S_StartSound( NULL, es->number, CHAN_AUTO, trap_S_RegisterSound( "sound/actors/player/gasp.wav", qfalse ) );
		}
		break;
	...
	case    EV_BLOOD_ON_WALL:
		...
		if ( es->legsAnim ) {
			...
			if ( ( cg.DeafTime < cg.time ) ) {
				trap_S_StartSound( NULL, es->otherEntityNum, CHAN_AUTO, trap_S_RegisterSound( head_sound, qfalse ) );
			}
		}
