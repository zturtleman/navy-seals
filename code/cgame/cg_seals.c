...
void CG_LockBreak( centity_t *cent ) {
	...
	if ( ( cg.DeafTime < cg.time ) ) {
		trap_S_StartSound( origin, cent->currentState.number, CHAN_AUTO, cgs.media.sfxMetal[ (int)( random() * 5 ) ] );
	}
}
...
gesamte breite des imaginren rechtecks - width - angegebenes X = richtiges X

hhe eines BACKGROUND STREIFEN mit zwischenabstand ist (24+2)=26
