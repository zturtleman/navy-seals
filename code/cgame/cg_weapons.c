...
void CG_InvenSelect( void ) {
	...
	if ( 1 ) {
		...
		if ( pw == PW_BRIEFCASE ) {
			...
			if ( ( cg.DeafTime < cg.time ) ) {
				trap_S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_ITEM, trap_S_RegisterSound( "sound/hud/select.wav",qfalse ) );
			}
			return;
		}
	}
	...
	if ( cg.curInventory == 666 ) {
		if ( ( cg.DeafTime < cg.time ) ) {
			trap_S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_ITEM, trap_S_RegisterSound( "sound/hud/close.wav",qfalse ) );
		}
		cg.curInventory = 666;
		cg.activeInventory = 0;
...
void CG_MWheel_f( int prev ) {
	...
	if ( !cg.activeInventory ) {
		...
		if ( ( cg.DeafTime < cg.time ) ) {
			trap_S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_ITEM, trap_S_RegisterSound( "sound/hud/open.wav",qfalse ) );
		}
	...
	if ( prev ) {
		...
		if ( ( cg.DeafTime < cg.time ) ) {
			trap_S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_ITEM, trap_S_RegisterSound( "sound/hud/moveselect.wav",qfalse ) );
		}
		return;
...
void CG_Weapon_f( void ) {
	...
	if ( cg.inMenu ) {
		...
		if ( ( cg.DeafTime < cg.time ) ) {
			trap_S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_LOCAL, cgs.media.noAmmoSound );
		}
...
void CG_ReloadWeapon( centity_t *cent, int last_rnd ) {
	...
	if ( ( cg.DeafTime < cg.time ) ) {
		trap_S_StartSound( NULL, ent->number, CHAN_WEAPON, weap->reloadEmptySound );
	}
}
...
void CG_Bullet( vec3_t end, vec3_t start, int sourceEntityNum, vec3_t normal, qboolean flesh, int fleshEntityNum, int damage, int soundtype, int bholetype, int weapon ) {
	...
	if ( flesh ) {
		...
		if ( ( cg.DeafTime < cg.time ) ) {
			trap_S_StartSound( end, fleshEntityNum, CHAN_AUTO, cgs.media.bulletHitFlesh[r] );
		}
