...
int BG_CalcSpeed( playerState_t ps ) {
	...
	if ( ( ps.velocity[2] > 2.0 ||
		...
		return SEALS_JUMPING;
	}

	// standing
	if ( spd < SEALS_SPEED_STANDING ) {
		return SEALS_STANDING;
	}

	// crouching
	if ( ps.pm_flags & PMF_DUCKED ) {
		return SEALS_CROUCHING;
	}

	// walking
	if ( spd < SEALS_SPEED_WALKING ) {
		return SEALS_WALKING;
	}

	// sprinting
	if ( ( spd >= SEALS_SPEED_RUNNING ) &&
		 ( spd < SEALS_SPEED_SPRINTING ) ) {
		return SEALS_SPRINTING;
	}
