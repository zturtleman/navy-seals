	...
	if ( mod == MOD_FALLING ) {
		...
		if ( dflags & DAMAGE_RADIUS ) {
			...
			if ( HitLocation == LOC_FACE ) {
				...
				spray_blood = qtrue;
				if ( NS_GotPowerup( targ, PW_HELMET ) ) {
					take *= 1;
				} else { take *= 2; }
				armorhit = 2;
				targ->client->ps.stats[STAT_HEAD_DAMAGE] += take * 0.8;
				...
				armorhit = 1;
				spray_blood = qtrue;
				if ( ( random() < 0.4 ) || NS_GotPowerup( targ, PW_VEST ) ) {
					bleeding = qfalse;
				} else { bleeding = qtrue; }
		...
		if ( NS_GotPowerup( targ, PW_VEST ) &&
			 ...
			 ( HitLocation == LOC_STOMACH ) ) {
			switch ( attacker->client->ps.weapon ) {
			case WP_GLOCK:
...
qboolean G_RadiusDamage( vec3_t origin, gentity_t *attacker, float damage, float radius, gentity_t *ignore, int mod ) {
	...
	for ( i = 0; i < level.num_entities ; i++ ) {
		...
		if ( !ent->inuse ) {
			continue;               // ignoring entities not in use
		}
		if ( ent == ignore ) {
			continue;                // ignoring as requested
		}
		if ( !ent->takedamage ) {
			continue;                    // cannot take damage, i'm not interested
		}
		// BLUTENGEL_XXX 08.01.2004
		// why are there lots of player_bbox_head entities????
		if ( !Q_stricmp( ent->classname, "player_bbox_head" ) ) {
			continue;                                                   // ignore the players head bounding box
		...
		// too far away
		if ( ( dist = VectorLength( oldOrg ) ) > radius ) {
			continue;
		}
