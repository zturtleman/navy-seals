...
void SP_info_bomb_disarm( gentity_t *self ) {
	...
	if ( defusable ) {
		self->ns_flags |= NS_FLAG_DEFUSABLE;
	}
...
void assault_link_all( qboolean unlink ) {
	...
	for ( i = 0 ; i < level.num_entities ; i++, ent++ )
	{
		if ( !Q_stricmp( "trigger_assaultfield", ent->classname ) ) { // this is an assaultfield
			if ( unlink ) {
				trap_UnlinkEntity( ent );
			} else {
				trap_LinkEntity( ent );
...
void Touch_GlassTrigger( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	...
	if ( other->r.maxs[2] >= glass->r.maxs[2] - 2 ) { // can 1be broken when stood upon
...
void Touch_Elevator_Button( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	...
	if ( other->client && ( other->client->buttons & BUTTON_USE ) && ( other->client->oldbuttons & BUTTON_USE ) ) {
		return;
	}
