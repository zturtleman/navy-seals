...
void Touch_DoorTrigger( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	...
	if ( other->client && Is_Spectator( other ) ) { // if the door is not open and not opening
		Touch_DoorTriggerSpectator( ent, other, trace );
	...
	if ( ( ent->moverState == MOVER_POS1 ||
		...
		while ( ( t = G_Find( t, FOFS( targetname ), ent->target ) ) != NULL ) {
			if ( ( other->elevator_index == t->elevator_index ) &&
				 ( !strcmp( t->classname, "path_elevator" ) ) ) {
				break;
			}
		}
...
void SP_path_elevator( gentity_t *self ) {
	...
	if ( self->spawnflags == 2 ) {
		self->elevator_index = 1;
	}
	if ( self->spawnflags == 4 ) {
		self->elevator_index = 2;
	}
	if ( self->spawnflags == 8 ) {
		self->elevator_index = 3;
	}
	if ( self->spawnflags == 16 ) {
		self->elevator_index = 4;
	}
	if ( self->spawnflags == 32 ) {
		self->elevator_index = 5;
	}
	if ( self->spawnflags == 64 ) {
		self->elevator_index = 6;
	}
	if ( self->spawnflags == 128 ) {
		self->elevator_index = 7;
	}
}
