...
int NS_CheckLocationDamage( gentity_t *targ, vec3_t point, int mod ) {
	...
	if ( g_debugDamage.integer == 1 ) {
		G_Printf( "Bulletheight: %f || Clientheight: %f || mod: %i\n", bulletHeight, clientHeight, mod );
	}
	...
	if ( ( bulletHeight > clientHeight + headMod ) ) {
		...
		if ( g_debugDamage.integer == 1 ) {
			G_Printf( "Stomachshot\n" );
		}
		return LOC_STOMACH;
	} else if ( inback == 2 || inback == 0 ) {
		if ( g_debugDamage.integer == 1 ) {
			G_Printf( "Rightlegshot\n" );
		}
		return LOC_RIGHTLEG;
	} else {
		if ( g_debugDamage.integer == 1 ) {
			G_Printf( "Leftlegshot\n" );
		}
		return LOC_LEFTLEG;
	}
...
gentity_t *NS_RandomPlayer( int ignoreClientNum, team_t team ) {
	...
	for ( i = ignoreClientNum + 1 ; i < level.maxclients ; i++ ) {
		...
		if ( level.clients[i].pers.lockedPlayer > 0 ) {
			continue;
		}
	...
	if ( currvip >= 0 ) {
		return &g_entities[ level.clients[currvip].ps.clientNum ];
	}
...
void CheckTeamplay( void ) {
	...
	if ( ( TeamCount( -1, TEAM_RED ) < minplayers ) || ( TeamCount( -1, TEAM_BLUE ) < minplayers )  ) {
		...
		if ( level.time > i_sNextWaitPrint ) { // not enough players
	...
	} else
	{
		...
		if ( level.warmupTime == level.time ) {
			...
			if ( random() < 0.5 ) {
				if ( bmbplayer != NULL ) {
					NS_BotRadioMsg( bmbplayer, "gogo" );
				} else
				if ( bmbplayer != NULL ) {
					NS_BotRadioMsg( bmbplayer, "locknload" );
				}
			}
...
float NS_CalcWeight( gentity_t *ent ) {
	...
	//
	if ( ent->client->ps.ammo[AM_SHOTGUN] > 0 ) {
		weight += ent->client->ps.ammo[AM_SHOTGUN] * SEALS_WEIGHT_SHOTGUN;
	}
	if ( ent->client->ps.ammo[AM_SHOTGUNMAG] > 0 ) {
		weight += ent->client->ps.ammo[AM_SHOTGUNMAG] * SEALS_WEIGHT_SHOTGUNMAG;
	}

	if ( ent->client->ps.ammo[AM_LIGHT_PISTOL] > 0 ) {
		weight += ent->client->ps.ammo[AM_LIGHT_PISTOL] * SEALS_WEIGHT_LIGHT_PISTOL;
	}
	if ( ent->client->ps.ammo[AM_MEDIUM_PISTOL] > 0 ) {
		weight += ent->client->ps.ammo[AM_MEDIUM_PISTOL] * SEALS_WEIGHT_MEDIUM_PISTOL;
	}
	if ( ent->client->ps.ammo[AM_LARGE_PISTOL] > 0 ) {
		weight += ent->client->ps.ammo[AM_LARGE_PISTOL] * SEALS_WEIGHT_LARGE_PISTOL;
	}
	if ( ent->client->ps.ammo[AM_SMG] > 0 ) {
		weight += ent->client->ps.ammo[AM_SMG] * SEALS_WEIGHT_SMG;
	}
	if ( ent->client->ps.ammo[AM_RIFLE] > 0 ) {
		weight += ent->client->ps.ammo[AM_RIFLE] * SEALS_WEIGHT_RIFLE;
	}
	if ( ent->client->ps.ammo[AM_MG] > 0 ) {
		weight += ent->client->ps.ammo[AM_MG] * SEALS_WEIGHT_MG;
	}
	if ( ent->client->ps.ammo[AM_MEDIUM_SNIPER] > 0 ) {
		weight += ent->client->ps.ammo[AM_MEDIUM_SNIPER] * SEALS_WEIGHT_MEDIUM_SNIPER;
	}
	if ( ent->client->ps.ammo[AM_LARGE_SNIPER] > 0 ) {
		weight += ent->client->ps.ammo[AM_LARGE_SNIPER] * SEALS_WEIGHT_LARGE_SNIPER;
	}
	if ( ent->client->ps.ammo[AM_GRENADES] > 0 ) {
		weight += ent->client->ps.ammo[AM_GRENADES] * SEALS_WEIGHT_GRENADE;
	}
	if ( ent->client->ps.ammo[AM_FLASHBANGS] > 0 ) {
		weight += ent->client->ps.ammo[AM_FLASHBANGS] * SEALS_WEIGHT_FLASHBANG;
	}
	if ( ent->client->ps.ammo[AM_SMOKE] > 0 ) {
		weight += ent->client->ps.ammo[AM_SMOKE] * SEALS_WEIGHT_SMOKE;
	}
	if ( ent->client->ps.ammo[AM_40MMGRENADES] > 0 ) {
		weight += ent->client->ps.ammo[AM_40MMGRENADES] * SEALS_WEIGHT_40MMGRENADE;
	}
...
void NS_PlayerInventory( gentity_t *ent ) {
	...
	*/
	...
	if ( i > -1 ) {
		...
		if ( ( grennum + num_gren ) <= SEALS_MAX_GRENADES ) {
			ent->client->pers.nsInven.ammo[ AM_FLASHBANGS ] = num_gren;
		} else {
			if ( BG_GotWeapon( WP_FLASHBANG, ent->client->ps.stats ) ) {
				BG_RemoveWeapon( WP_FLASHBANG, ent->client->ps.stats );
			}
	...
	if ( i > -1 ) {
		...
		if ( ( grennum + num_gren ) <= SEALS_MAX_GRENADES ) {
			ent->client->pers.nsInven.ammo[ AM_SMOKE ] = num_gren;
		} else {
			if ( BG_GotWeapon( WP_SMOKE, ent->client->ps.stats ) ) {
				BG_RemoveWeapon( WP_SMOKE, ent->client->ps.stats );
			}
