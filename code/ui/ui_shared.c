...
void Window_Paint(Window *w, float fadeAmount, float fadeClamp, float fadeCycle) {
    ...
    if (w->border == WINDOW_BORDER_FULL) {
        ...
        {
#ifdef CGAME 
            DC->drawRect(w->rect.x, w->rect.y, w->rect.w, w->rect.h, w->borderSize, DC->Assets.dmcolor);
        ...
        if (key == K_MOUSE1 || key == K_ENTER || key == K_MOUSE2 || key == K_MOUSE3) {
            ...
            if (editDef) {
                ...
                if (Rect_ContainsPoint(&testRect, DC->cursorx, DC->cursory)) {
                    ...
                    value = work / width;
                    value *= (editDef->maxVal - editDef->minVal);
					/* what the fuck is it with those comments, the TA team must have been on crack */
					/* damn nigras */
                    // vm fuckage
...
void Item_Text_Paint(itemDef_t *item) {
	...
	// defcon: who the hell made this mess then?
    /*
    adjust = 0;
...
void Item_Slider_Paint(itemDef_t *item) {
    ...
    if ( novscreen )
        DC->drawStretchPic( x, y-1, SLIDER_WIDTH, SLIDER_HEIGHT, 0, 0, 1, 1, DC->Assets.sliderBar );
    else
    ...
    if ( x > 640 )
        DC->drawStretchPic(  x - (SLIDER_THUMB_WIDTH / 2), y - 1 , SLIDER_THUMB_WIDTH, SLIDER_THUMB_HEIGHT, 0, 0, 1, 1, DC->Assets.sliderThumb );
    else
        DC->drawHandlePic( x - (SLIDER_THUMB_WIDTH / 2), y - 1 , SLIDER_THUMB_WIDTH, SLIDER_THUMB_HEIGHT, DC->Assets.sliderThumb );
